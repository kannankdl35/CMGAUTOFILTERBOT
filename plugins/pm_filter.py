...
from database.ia_filterdb import col, sec_col, db as vjdb, sec_db, get_file_details, get_search_results, get_bad_files, get_fuzzy_search_results
...
async def next_page(bot, query):
   ...
   # if not search:
    ...
    if not files:
        # The original query may have been a typo that only matched via fuzzy
        # search on the first page — keep using fuzzy matching for later pages too.
        files, n_offset, total = await get_fuzzy_search_results(query.message.chat.id, search, offset=offset)
    try:
    ...
    if settings['button']:
        ...
        btn.insert(0, [
            InlineKeyboardButton("♻️ Sent All", callback_data=f"sendfiles#{key}"),
...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="⇊ ꜱᴇʟᴇᴄᴛ ʏᴏᴜʀ 📁 Quality ⇊", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            await query.answer('Uɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ ꜰɪʀꜱᴛ!', show_alert=True)
     ...
     #   if not search:
      ...
      #      return
    ...
    elif query.data.startswith("reqres#"):
        _, action, from_user = query.data.split("#")
        ...
        # E) Abort here if the requester never started the bot — there's no way to DM them.
        if not await db.is_user_exist(from_user):
            await query.answer("⚠️ This user hasn't started the bot yet, so a status update can't be sent to them. Action aborted.", show_alert=True)
            return

        try:
            user = await client.get_users(from_user)
        except Exception as e:
            await query.answer(f"Couldn't fetch this user: {e}", show_alert=True)
            return

        # Pull the original request text back out of the request-channel post.
        original_text = query.message.text or ""
        marker = "🎬 Request:\n"
        req_query = original_text.split(marker, 1)[1].strip() if marker in original_text else "your request"
        ...
        if action == "avail":
            dm_text = (
                f"Hey {name} 👋,\n\n"
                f"We reviewed your request, and \"{req_query}\" is already available on our bot.\n\n"
                f"Maybe the spelling you sent was incorrect. Please make sure you enter the correct spelling of the movie. "
                f"You can copy the exact spelling from Google or from our @cmg_movieinfo_bot and try again.\n\n"
                f"If you need any further help, please contact @cmg_4dmin.\n\n"
                f"Thank you! ❤️"
            )
            status_label = "✅ Already Available"
            alert_text = "Marked as Already Available ✅"

        elif action == "notott":
            dm_text = (
                f"Hey {name} 👋,\n\n"
                f"We reviewed your request, and \"{req_query}\" has not been released on OTT yet.\n\n"
                f"Please note that our bot can only provide movies that are already available on OTT platforms. "
                f"So, please wait until the movie is released on OTT and try again. \n\n"
                f"If you need any further help, please contact @cmg_4dmin.\n\n"
                f"Thank you! ❤️"
            )
            status_label = "🕒 Not Yet On OTT"
            alert_text = "Marked as Not Yet On OTT 🕒"

        elif action == "added":
            dm_text = (
                f"Hey {name} 👋,\n\n"
                f"We reviewed your request, and \"{req_query}\" has been added to the bot. 🎉\n\n"
                f"You can now find \"{req_query}\" by sending the correct spelling to the bot. Thank you for reporting the issue and helping us improve the bot! \n\n"
                f"If you need any further help, please contact @cmg_4dmin.\n\n"
                f"Thank you! ❤️"
            )
            status_label = "📤 Added To Bot"
            alert_text = "Marked as Added To Bot 🎉"
        ...
        try:
            await client.send_message(chat_id=from_user, text=dm_text)
        except UserIsBlocked:
            await query.answer("⚠️ This user has blocked the bot — couldn't deliver the status update.", show_alert=True)
            return
        except Exception as e:
        ...
        try:
            await query.message.edit_text(
                f"<s>{original_text}</s>\n\n{status_label} — handled by {query.from_user.mention}",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(status_label, callback_data="reqres_done")]])
            )
        except MessageNotModified:
            pass
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            settings = await get_settings(message.chat.id)
            if not files:
                # Exact search found nothing — before giving up, look for the closest
                # matching file names in our own database (handles typos like a missing
                # or swapped letter) without needing any external lookup.
                fuzzy_files, fuzzy_offset, fuzzy_total = await get_fuzzy_search_results(message.chat.id, search, offset=0)
                if fuzzy_files:
    ...
    if imdb:
        ...
        if settings["button"]:
            cap = f"<i>• The Request For ➢ {search}\n\n• Requested By ➢ {message.from_user.mention}\n\n• Result Shown In ➢ {remaining_seconds} seconds\n\n• Powered By ➢ : {message.chat.title} \n\n⚠️ After 5 Minutes This Message Will Be Automatically Delete. 🗑️\n\n</i>"
        else:
            cap = f"<i>• The Request For ➢ {search}\n\n• Requested By ➢ {message.from_user.mention}\n\n• Result Shown In ➢ {remaining_seconds} seconds\n\n• Powered By ➢ : {message.chat.title} \n\n⚠️ After 5 Minutes This Message Will Be Automatically Delete. 🗑️\n\n</i>"
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    try:
        ...
        button = [
            [InlineKeyboardButton("Search Spelling on Google", url=f"https://www.google.com/search?q={reqst_gle}")],
            [InlineKeyboardButton("Search Spelling on Movie Info Bot", url="https://t.me/CMG_MovieInfo_Bot")]
        ]
        if NO_RESULTS_MSG:
