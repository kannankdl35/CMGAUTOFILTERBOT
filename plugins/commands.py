...
from info import CLONE_MODE, OWNER_LNK, REACTIONS, CHANNELS, REQUEST_TO_JOIN_MODE, TRY_AGAIN_BTN, ADMINS, PREMIUM_AND_REFERAL_MODE, STREAM_MODE, AUTH_CHANNEL, REFERAL_PREMEIUM_TIME, REFERAL_COUNT, PAYMENT_TEXT, PAYMENT_QR, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT, MAX_B_TN, URL
...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        buttons = [[
                ...
                InlineKeyboardButton('📽️ 𝘼𝘿𝘿 𝙈𝙀 𝙏𝙊 𝙔𝙊𝙐𝙍 𝙂𝙍𝙊𝙐𝙋 📽️', url=f'http://t.me/{temp.U_NAME}?startgroup=true&admin=invite_users')
            
            ],[

                InlineKeyboardButton('🍿 𝙅𝙊𝙄𝙉 𝙈𝙊𝙑𝙄𝙀 𝙂𝙍𝙊𝙐𝙋 🍿', url=GRP_LNK)

            ],[

                InlineKeyboardButton('🎞️ 𝙉𝙀𝙒 𝙍𝙀𝙇𝙀𝘼𝙎𝙀𝙎 🎞️', url=CHNL_LNK),

                InlineKeyboardButton('🔗 𝘾𝙃𝘼𝙉𝙉𝙀𝙇 𝙇𝙄𝙉𝙆𝙎 🔗', url=f'https://t.me/{SUPPORT_CHAT}')

            ],[

                InlineKeyboardButton('💬 𝘾𝙊𝙉𝙏𝘼𝘾𝙏 𝘼𝘿𝙈𝙄𝙉 💬', url=OWNER_LNK)
...
@Client.on_message((filters.command(["request", "Request"]) | filters.regex(r"(?i)(^|\s)#request\b")) & (filters.group | filters.private) & filters.incoming)
async def requests(bot, message):
    ...
    # Pull out the actual request text, either from a replied-to message
    # or from the command/hashtag text itself.
    ...
    source = "Gʀᴏᴜᴘ" if message.chat.type != enums.ChatType.PRIVATE else "Pʀɪᴠᴀᴛᴇ Cʜᴀᴛ"
    ...
    buttons = [
        [
            InlineKeyboardButton("✅ Already Available", callback_data=f"reqres#avail#{user.id}"),
            InlineKeyboardButton("🕒 Not Yet On OTT", callback_data=f"reqres#notott#{user.id}")
        ],
        [
            InlineKeyboardButton("📤 Added To Bot", callback_data=f"reqres#added#{user.id}")
        ]
    ]
    ...
    if message.chat.type == enums.ChatType.PRIVATE:
        # B) Request made directly in the bot's PM.
        reply_text = (
            f"Heyy {name} 👋,\n\n"
            f"Your request to add \"{query_text}\" has been sent to the admins. Our admins will review your request and notify you about its status through our movie bot.\n\n"
            f"Thank you for your support! ❤️"
        )
        await message.reply_text(reply_text)
    else:
        # A) Request made in a group.
        reply_text = (
            f"Heyy {name} 👋,\n\n"
            f"Your request to add \"{query_text}\" has been sent to the admins. Our admins will review your request and notify you about its status through our movie bot.\n\n"
            f"⚠️ <i>Make sure you have already started our @{temp.U_NAME}</i>. Otherwise, the admins won't be able to send you the status update.\n\n"
            f"Thank you for your support! ❤️"
        )
        start_btn = [[InlineKeyboardButton("🤖 Start The Bot", url=f"https://t.me/{temp.U_NAME}")]]
        await message.reply_text(reply_text, reply_markup=InlineKeyboardMarkup(start_btn))
